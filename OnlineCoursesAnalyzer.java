...
public class OnlineCoursesAnalyzer {
  ...
  public static class Course6 {
    ...
    public Course6(String courseNum, String launchDate, String courseName, double averageUserAge,
                   double averageMaleRate, double averageBachelorRate) {
  ...
  public Map<String, Integer> getPtcpCountByInstAndSubject() {
    Stream<Course> courseStream = data.stream()
        .map(row -> new Course(row[0] + "-" +
            (row[5].contains("\"") ? row[5].substring(1, row[5].length() - 1) : row[5]),
  ...
  public Map<String, List<List<String>>> getCourseListOfInstructor() {
    ...
    for (String[] row : data) {
      String[] instructors =
          (row[4].contains("\"") ? row[4].substring(1, row[4].length() - 1) : row[4]).split(
              ","); // 获取讲师姓名列表


      String courseName =
          (row[3].contains("\"") ? row[3].substring(1, row[3].length() - 1) : row[3]); // 获取课程名称
...
//        otherCourse.get()
  ...
  public List<String> getCourses(int topK, String by) {
    ...
    if (by.equals("hours")) {
      comparator = Comparator.comparing(Course4::getTotalHours).reversed()
          .thenComparing(Comparator.comparing(Course4::getCourseName));
    } else if (by.equals("participants")) {
      comparator = Comparator.comparing(Course4::getParticipants).reversed()
          .thenComparing(Comparator.comparing(Course4::getCourseName));
    } else {
  ...
  public List<String> searchCourses(String courseSubject, double percentAudited,
    ...
    List<String> courses = data.stream()
        ...
        .map(row -> (row[3].contains("\"") ? row[3].substring(1, row[3].length() - 1) :
            row[3]))        //同 p4，这个应该要返回的是名字而非课程代码
        .distinct()
        .sorted()
        .toList();
  ...
  public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) {
    List<Course6> course6s = data.stream().map(
        row -> new Course6(row[1], row[2], row[3], Double.parseDouble(row[19]),
            Double.parseDouble(row[20]), Double.parseDouble(row[22]))).toList();
    Map<String, List<Course6>> courseGroups =
        course6s.stream().collect(Collectors.groupingBy(Course6::getCourseNum));


    Map<String, Double> averageUserAge = courseGroups.entrySet().stream().collect(Collectors.toMap(
        Map.Entry::getKey,
        entry -> entry.getValue().stream()
            .collect(Collectors.averagingDouble(Course6::getAverageUserAge))
    ));

    Map<String, Double> averageMaleRate = courseGroups.entrySet().stream().collect(Collectors.toMap(
        Map.Entry::getKey,
        entry -> entry.getValue().stream()
            .collect(Collectors.averagingDouble(Course6::getAverageMaleRate))
    ));


    Map<String, Double> averageBachelorRate =
        courseGroups.entrySet().stream().collect(Collectors.toMap(
            Map.Entry::getKey,
            entry -> entry.getValue().stream()
                .collect(Collectors.averagingDouble(Course6::getAverageBachelorRate))
        ));
...
//                if(courseNum.contains("15.071x")){
      ...
      double similarity = Math.pow(avgAge - age, 2) + Math.pow(avgGender - gender * 100, 2) +
          Math.pow(avgBachel - isBachelorOrHigher * 100, 2);
